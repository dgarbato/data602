...
def exercise08():
    ...
    filter1 = df['zip'] == 95670
    df_zip_95670 = df[filter1]

    filter2 = df['zip'] != 95610
    df_zip_not_95610 = df[filter2]
...
def exercise12(n):
    ...
    row1 = [1, 0] * n
    row2 = [0, 1] * n
    checkerboard_matrix = np.array([row1, row2] * n)
...
def exercise13(n):
    ...
    data = np.random.randint(0, n + 1, size=n)
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df1 = pd.read_csv("https://tinyurl.com/y63q7okz")
    df2 = df1.iloc[::5, [0, 2]]
    df = df2
